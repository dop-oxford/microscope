...
class _GalvoScanner(microscope.abc.Stage):
    def __init__(
        ...
        if len(axes_names) < num_axes:
            raise ValueError(
                "Number of axis names must be at least equal to num_axes"
            )
        self.num_axes = num_axes

        self._axes = {
            axes_names[i]: _GalvoAxis(axes_names[i]) for i in range(num_axes)
        }

        # Handle linear_conversion input
        if isinstance(linear_conversion, (int, float)):
            self.linear_conversion = {
                axis: linear_conversion for axis in self._axes
            }
        elif isinstance(linear_conversion, list):
            if len(linear_conversion) != num_axes:
                raise ValueError(
                    "Length of linear_conversion must match num_axes"
                )
            self.linear_conversion = {
                axis: conv for axis, conv in zip(self._axes, linear_conversion)
            }
        else:
            self.linear_conversion = {
                axis: 1 for axis in self._axes
            }  # Default value
        ...
        self.position_lin = {
            axis: self.angle_to_linear(axis, self.position_ang[axis])
    ...
    def _set_angles(self, angles):
        ...
        if len(angles) != self.num_axes:
            raise ValueError(
                f"Axes {self._axes} cannot be set with angles {angles}(expected {self.num_axes} angles)"
            )
    ...
    def validate_angles(self, angles, axis=None):
        ...
        if isinstance(angles, (float, int)):  # Single angle provided
            if axis is None:
                raise ValueError(
                    "Axis must be provided for a single angle validation."
                )
            if isinstance(axis, int):
                if axis < 0 or axis >= self.num_axes:
                    raise ValueError(
                        f"Axis index '{axis}' is out of range. Must be between 0 and {self.num_axes - 1}."
                    )
            ...
            if (
                not self.angle_limits[axis_name][0]
                <= angles
                <= self.angle_limits[axis_name][1]
            ):
                raise ValueError(
                    f"Angle {angles} out of allowed range for axis {axis_name}"
                )
            return angles
            ...
            for ax, ang in angles.items():
                if isinstance(ax, int):
                    if ax < 0 or ax >= self.num_axes:
                        raise ValueError(
                            f"Axis index '{ax}' is out of range. Must be between 0 and {self.num_axes - 1}."
                        )
                    ax = self._axes[ax]  # Convert index to name
                self.validate_axes(ax)  # Validates each axis name
                if (
                    not self.angle_limits[ax][0]
                    <= ang
                    <= self.angle_limits[ax][1]
                ):
                    raise ValueError(
                        f"Angle {ang} out of allowed range for axis {ax}"
                    )
                validated_angles[ax] = ang
            return validated_angles
        else:
            raise TypeError(
                "Angles must be a single float/int (with `axis` provided) or a dictionary of axis-angle pairs."
            )
        ...
        if isinstance(axes, (str, int)):
            ...
            if isinstance(axes, int):
                if axes < 0 or axes >= self.num_axes:
                    raise ValueError(
                        f"Axis index '{axes}' is out of range. Must be between 0 and {self.num_axes - 1}."
                    )
            elif axes not in self._axes:
                raise ValueError(
                    f"Invalid axis name '{axes}', valid axes are {self._axes}"
                )
            return axes
            ...
            if all(isinstance(axis, int) for axis in axes):
                ...
                raise TypeError(
                    "All axes must be of the same type, either int or str."
                )
            if len(set(axes)) != len(axes):
                raise ValueError(
                    "Duplicate axes found. Each axis must be unique."
                )
            for axis in axes:
                if isinstance(axis, str) and axis not in self._axes:
                    raise ValueError(
                        f"Invalid axis name '{axis}', valid axes are {self._axes}"
                    )
            return axes
        else:
            # Invalid input type
            raise TypeError(
                "Axes must be either a single axis name (str) / index (int) or a list of axis names / indices (list of str/int)."
            )
    ...
    def get_position(self, axes=None, verbose=True):
        ...
        for axis in axes:
            self.validate_axes(
                axis
            )  # Ensure the axis is valid. Note: validate_axis might need to handle int indices too.
            positions[axis] = self.get_angle_ax(
                axis
            )  # Retrieve and store the position for the axis.
    ...
    def move_to_position(
        self, axis, angle, relative=False, timeout_s=5000, tolerance=0.1
    ):
        ...
        if len(axis) != len(angle):
            raise ValueError(
                "Axis and angle parameters must have the same length."
            )
        ...
        for ax, ang in zip(axis, angle):
            ...
            self.set_angle(
                ax, target_angle
            )  # Assume set_angle has been adjusted to immediately set the angle
        ...
        while True:
            ...
            if (time.time() - start_time) > timeout_s:
                ...
                raise TimeoutError(
                    f"Timeout reached, motion not finished after {timeout_s} s. "
                    f"Current positions: {current_angles}, target positions: {self._target_angle}."
                )
            time.sleep(
                0.01
            )  # Small delay to prevent spamming the _motion_finished function
...
class GVS201_Controller(_GalvoScanner):
    def __init__(
        ...
        super().__init__(
            num_axes=1,
            linear_conversion=linear_conversion,
            angle_limits=[(-12.5, +12.5)],
            axes_names=axis_name,
            verbose=verbose,
        )
        accepted_voltage_to_deg_set = [
            0.5,
            0.8,
            1,
        ]  # Accepted values for GVS201 (see manual)
        if voltage_to_deg_set not in accepted_voltage_to_deg_set:
            raise ValueError(
                f"Accepted values for voltage_to_deg_set are {accepted_voltage_to_deg_set}"
            )
        self._voltage_to_deg_set = voltage_to_deg_set
        self._voltage_to_deg_get = (
            0.5  # Always 0.5 v/deg for GVS201 (see manual)
        )
        self._input_voltage_limits = (
            -10,
            10,
        )  # Input voltage range of GVS201 (see manual)
    ...
    def validate_voltage(self, voltage):
        ...
        """
        if (
            not self._input_voltage_limits[0]
            <= voltage
            <= self._input_voltage_limits[1]
        ):
            raise ValueError(
                f"Voltage {voltage} out of allowed range for GVS201"
            )
    ...
    def get_scanner_position_deg(self):
        ...
        """
        return (
            self.get_diagnosis_connector_pin_voltage(pin=1)
            * self._voltage_to_deg_get
        )
    ...
    def get_internal_command_signal_deg(self):
        ...
        """
        return (
            self.get_diagnosis_connector_pin_voltage(pin=2)
            * self._voltage_to_deg_get
        )
    ...
    def get_position_error_deg(self):
        ...
        """
        return (
            self.get_diagnosis_connector_pin_voltage(pin=3)
            * self._voltage_to_deg_get
            / 5
        )
