"""
tl_polarization_processor.py ***BETA***
"""
...
class PolarizationProcessorSDK(object):
    ...
    def __init__(self):
        ...
        try:
            if platform.system() == "Windows":
                self._sdk = cdll.LoadLibrary(
                    r"thorlabs_tsi_polarization_processor.dll"
                )
            elif platform.system() == "Linux":
                try:
                    self._sdk = cdll.LoadLibrary(
                        r"./libthorlabs_tsi_polarization_processor.so"
                    )
                except OSError:
                    self._sdk = cdll.LoadLibrary(
                        r"libthorlabs_tsi_polarization_processor.so"
        ...
        try:
            ...
            self._sdk.tl_polarization_processor_destroy_polarization_processor.argtypes = [
                c_void_p
            ]
    ...
    def dispose(self):
        ...
        try:
            ...
            _logger.error(
                "Polarization SDK destruction failed; " + str(exception)
            )
            raise exception
        ...
        try:
            ...
            if error_code != 0:
                raise PolarizationError(
                    _create_c_failure_message(
                        self._sdk,
                        "tl_polarization_processor_create_polarization_processor",
                        error_code,
                    )
                )
            # noinspection PyProtectedMember
            return PolarizationProcessor._create(
                self._sdk, c_polarization_handle
            )
        except Exception as exception:
            _logger.error(
                "Failed to create polarization processor; " + str(exception)
            )
            raise exception
...
class PolarizationProcessor(object):
    ...
    def _create(cls, sdk, polarization_processor_handle):
        ...
        return PolarizationProcessor(
            cls.__key, sdk, polarization_processor_handle
        )
    ...
    def __init__(self, key, sdk, polarization_processor_handle):
        ...
        try:
            ...
            _logger.error(
                "PolarizationProcessor initialization failed; "
                + str(exception)
            )
            raise exception
        ...
        try:
            ...
            if error_code != 0:
                raise PolarizationError(
                    _create_c_failure_message(
                        self._sdk,
                        "tl_polarization_processor_destroy_polarization_processor",
                        error_code,
                    )
                )
            self._disposed = True
            ...
            _logger.error(
                "Could not dispose PolarizationProcessor instance; "
                + str(exception)
            )
            raise exception

    def transform_to_intensity(
        self,
        sensor_polar_phase,
        input_image,
        ...
        try:
            output_buffer = np.zeros(
                shape=(image_width_pixels * image_height_pixels,),
                dtype=c_ushort,
            )
            output_buffer_pointer = output_buffer.ctypes.data_as(
                POINTER(c_ushort)
            )
            input_buffer_pointer = input_image.ctypes.data_as(
                POINTER(c_ushort)
            )
            ...
            if error_code != 0:
                raise PolarizationError(
                    _create_c_failure_message(
                        self._sdk,
                        "tl_polarization_processor_transform",
                        error_code,
                    )
                )
            ...
            _logger.error(
                "Could not transform to intensity; " + str(exception)
            )
            raise exception

    def transform_to_dolp(
        self,
        sensor_polar_phase,
        input_image,
        image_origin_x_pixels,
