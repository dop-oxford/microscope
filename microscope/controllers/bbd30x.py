...
import os
import time
# # Necessary for real world units
...
class BBD30XController:
    ...
    def __init__(self, config):
        """
        Initialise the BBD30XController.

        Args:
            config (str or dict): Configuration for the controller. It can be a
                path to a config file (JSON) or a dictionary.
        """
        ...
        self.serialNo = config.get('serialNumber')
    ...
    def print_position(self):
        ...
        print(f"Device position: channels {self.channel_names} - {self.get_position_float()} mm")
...
if __name__ == '__main__':
    config = {
        # Serial number for Benchtop Brushless Motor (Example), None if not
        # known
        'serialNumber': '103251384',
        # Name on controller
        'name': 'BBD302',
        # Defines the name of the axis (e.g. 'X', 'Y', etc.)
        'channel_names': ('X', 'Y'),
        # States if the axis are reversed (i.e. left is positive)
        'reverse': 2 * (False,),
        # If True, info in printed in terminal
        'verbose': True,
        # If True, more info in printed in terminal
        'very_verbose': True,
        # If True, the code is to run a Kinesis simulation
        'isSim': False
    }
    XY_controller = BBD30XController(config)
    XY_controller.standard_initialize_channels(verbose=True)
    XY_controller.print_velocity_params()
    XY_controller.set_velocity_params(max_vel=200, acc=1100, verbose=True)
    XY_controller.print_velocity_params()
    # XY_controller.moveTo(
    #     np.array([53.452546,30.3564564]), max_vel=10, acc=200, verbose=True
    # )
